...
def send_message(notification_text):
    ...
    acct_b = sts_connection.assume_role(
        RoleArn="arn:aws:iam::{ACCOUNT_ID}:role/sns-cross-account-role",
    ...
    # Replace with the ARN of the target SNS topic
    topic_arn = 'arn:aws:sns:us-east-1:514811724234:publish-from-different-account'

    # # Replace with the message you want to publish to the topic
    # message = 'Alert! An object of interest was detected in the video feed! '

    # Create an SNS client in the target region
    sns = role1_session.client('sns', region_name='us-east-1')

    print(notification_text)


    # Publish the message to the target SNS topic
...
def process_image(event, context):
    ...
    for record in event['Records']:
        ...
        if len(labels_on_watch_list) > 0 \
            ...
            resp = send_message(notification_txt)
            print(resp)

            # if label_watch_phone_num:
            #     resp = sns_client.publish(PhoneNumber=label_watch_phone_num, Message=notification_txt)
            #     print(resp)
